...
if echo "$scan" | grep -qE '\bgit\b' && echo "$scan" | grep -qE '(^|[[:space:]])HUSKY=0|(^|[[:space:]])HUSKY_SKIP_HOOKS=|core\.hooksPath[[:space:]]*='; then
  echo "BLOCKED: HUSKY=0 / HUSKY_SKIP_HOOKS / 'git -c core.hooksPath=...' disables the husky hook layer (pre-commit, commit-msg, or pre-push — pre-push is where harness-integrity + the full test suite live) — the same bypass as --no-verify. Fix the failure instead." >&2
  exit 2
fi
