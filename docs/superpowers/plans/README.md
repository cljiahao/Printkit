...
- `2026-09-20-printer-connectors-plan5-vendor-ui.md` — "Printer connectors — Phase 5 (vendor UI)": the screens a vendor actually meets — the Printers page, the filterable picker with tap-to-open explanations on every badge, a setup wizard per kind of printer ending in a live "Connected", and the public Bluetooth guide that opens by not recommending itself. Records that no screenshots were taken, because printkit's dev server needs Supabase keys this working copy does not have.
